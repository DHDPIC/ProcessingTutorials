...
void draw() {
  ...
  for(int ly=0; ly<height; ly += 90) {
    for(int lx =0; lx<width/2; lx += 90) {
      fill( getVariation(cc) );
      ellipse(lx, ly, scaleL, scaleL);
    }
  }
  //
  //fill(getCompliment(cc));
  //ellipse(width*0.7, height/2, scaleR, scaleR);
  for(int ry=0; ry<height; ry += 90) {
    for(int rx =width/2; rx<width; rx += 90) {
      color t = getCompliment(cc);
      fill( getVariation(t) );
      ellipse(rx, ry, scaleR, scaleR);
    }
  }
...
color getVariation(color c) {
  float r = red(c);
  float g = green(c);
  float b = blue(c);

  return color(r+random(-51,51), g, b);
}
